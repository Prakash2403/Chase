...
from config import *
from errors import UnknownModeException
...
def get_data_from_quandl(stocks, save=True):
    ...
    for stock in stocks:
        ...
        if len(df_list) == 1:   # TODO: Temporary fix. Change it to work with multiple stocks.
            return df_list[0]
        return df_list
...
def get_datasets():
    if str(MODE).upper() == 'LOCAL':
        ...
        return [read_csv(DATA_DIR + stock.split('/')[-1] + '.csv', usecols=rel_cols, index_col=INDEX_COLUMN,
                         infer_datetime_format=True, parse_dates=True) for stock in STOCKS][0]
        # TODO: Temporary fix in above line. Change it to work with multiple stocks.
